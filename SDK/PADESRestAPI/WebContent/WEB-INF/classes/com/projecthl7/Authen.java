...
public class Authen {
  ...
  public boolean checklogin(String user,String pass) throws SQLException
  {
	  ...
	  ResultSet result = select.executeQuery("select username,password from ProviderStaff where username = '"+user1+"' and password = '"+hashword+"' and status=2");
	  if(result.next())
	  {
	      connection.close(); 
		  return true;
